...
\begin{theorem}{Dudley's Entropy Integral bound}{dudley_entropy_integral}
    ...
    \begin{align*}
        \ERC_S(\F) \le \inf_{\alpha>0} \biggRound{
            \frac{4\alpha}{\sqrt n} + \frac{12}{n}\int_\alpha^{\sqrt n} \sqrt{
...
\begin{proof*}[Proposition \ref{prop:rc_bound_for_lin_func}]
    ...
    \begin{align*}
        \log\mathcal{N}(\F, \epsilon, \|.\|_2) \le \Bigg\lceil \frac{R^2}{\epsilon^2} \Bigg\rceil \log(2d+1) \approx \frac{R^2}{\epsilon^2}\log D \ \ \ (\text{For some constant }D)
    \end{align*}

    \noindent Hence, from theorem \ref{thm:dudley_entropy_integral}, we have:
    \begin{align*}
        \int_\alpha^{\sqrt n} \sqrt{
            \log\mathcal{N}(\F, \epsilon,\|.\|_2) 
        }d\epsilon 
            &\approx R\sqrt{\log D} \int_\alpha^{\sqrt n} \frac{1}{\epsilon}d\epsilon \\
            &= R\sqrt{\log D}\biggRound{\log\sqrt{n} - \log\alpha}
    \end{align*}
