...
\textbf{1. Bounding the covering number of $\mathcal{L}_r$} : Let $L_\vecbf{w} \in \mathcal{L}_r$ where
\begin{align*}
...
\noindent And for any $h\in \Hf$ parameterized by $\beta=\begin{pmatrix} \vecbf{w_1} &\dots &\vecbf{w_K}\end{pmatrix}$, there exists $\bar h$, parameterized by $\bar\beta=\begin{pmatrix} \vecbf{\bar w_1} &\dots &\vecbf{\bar w_K}\end{pmatrix}$, such that $\|h - \bar h\|_\infty \le \epsilon$. Hence, we have:
\begin{align*}
...
\noindent\newline\textbf{2. Dudley's Entropy Integral} : 