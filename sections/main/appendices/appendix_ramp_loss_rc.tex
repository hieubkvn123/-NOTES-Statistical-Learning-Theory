...
\noindent Denote $C^{(j)}_\epsilon$ as the minimum $\epsilon$-cover of ${\F_j}_{|S}$. Meaning, for all $f_\vecbf{w} \in \F$:
\begin{align*}
    \exists v_j \in C^{(j)}_\epsilon : \|f_\vecbf{w}^{(j)} - v_j \|_\infty = \sup_{i \in \{1, \dots, n \}}\bigAbs{f_\vecbf{w}^{(j)}(x_i) - v_j} < \epsilon
\end{align*}
...
\begin{align*}
    \exists \begin{pmatrix}
        v_1 & \dots & v_K
    \end{pmatrix}^T \in C^{(1)}_\epsilon \times \dots \times C^{(K)}_\epsilon : \max_{j\in \{1, \dots, K \}} \|f_\vecbf{w}^{(j)} - v_j\|_\infty = \max_{
        \substack{ j \in \{1, \dots, K \}\\ i \in \{1, \dots, n\}}
    }\bigAbs{
        f_\vecbf{w}^{(j)}(x_i) - v_j
    } < \epsilon
\end{align*}

\noindent Since $C^{(j)}_\epsilon\subset {\F_j}_{|S}$ for all $j\in\{1,\dots, K\}$, there exists $f_\vecbf{\bar w}\in \F$  such that $v_j=f_\vecbf{\bar w}^{(j)}(x_i),  \ \forall j\in\{1,\dots, K\}$. Therefore, for all $f_\vecbf{w}\in\F$:
\begin{align*}
    \exists f_\vecbf{\bar w}(x_i) \in C^{(1)}_\epsilon \times \dots \times C^{(K)}_\epsilon : \max_{
        \substack{ j \in \{1, \dots, K \}\\ i \in \{1, \dots, n\}}
    }\bigAbs{
...
\begin{align*}
    ...
    &= 4\alpha + 144R\sqrt{\frac{K}{n}}\int_\alpha^1 \frac{1}{\epsilon}\sqrt{
        \log\biggRound{
            \biggRound{\frac{16R}{r\epsilon} + 7}n
        } 
    }d\epsilon \\
    &\le 4\alpha + 144R\sqrt{\frac{K}{n}}(1-\alpha)\sqrt{
        \log\biggRound{
            \biggRound{\frac{16R}{r} + 7}n
        } 
    } \ \ \ \text{(Setting $\epsilon=1$)}
\end{align*}

\noindent Let $\alpha \to 0$, we have:
\begin{align*}
    \ERC_S(\mathcal{L}_r) &\le 144R\sqrt{\frac{K}{n}}\sqrt{
        \log\biggRound{
            \biggRound{\frac{16R}{r} + 7}n
        } 
    } = \tilde O\biggRound{R\sqrt{\frac{K}{n}}}
\end{align*}
