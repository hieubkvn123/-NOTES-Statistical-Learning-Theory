...
\noindent However, note that even though $f_\vecbf{w}\in C^{(1)}_\epsilon \times \dots \times C^{(K)}_\epsilon$, it does not necessarily mean $f_\vecbf{w}\in\F$. Because in the worst case senario, we have:
\begin{align*}
    \|\vecbf{\bar w}\|_F &= \sqrt{\sum_{j=1}^K \|\vecbf{w}_j\|_2} \le \sqrt{\sum_{j=1}^K R^2} = R\sqrt{K}
\end{align*}
